...
@Component({
    selector: 'app-callRecordsCdrOb',
    templateUrl: 'callRecordsCdrOb.component.html',
    styleUrls: ['callRecordsCdrOb.component.less']
})

export class CallRecordsCdrObComponent implements OnInit {
    @ViewChild('callRecordsCdrOb-area') myDiv:ElementRef
    callRecordsCdrOb: any[] = [] /**table数据**/
    callRecordsCdrObTotal: any = {billDurationsTotal:0,bridgeDurationsTotal:0}
    params: any = {limit: 10, start: 0, callType: 1} /**请求通话记录接口的参数**/
        ...
        this.callRecordService.reloadcallRecords(this.params)
            .subscribe(page => {
                ...
                if (page.data.result === '0') {
                    ...
                    this.isSpinner = 0
                    this.callRecordsCdrOb = [...page.data.list]
    ...
    /*试听录音 同时打开录音播放器*/
    listenRecord(row){
        this.recordSrc = this.sanitizer.bypassSecurityTrustResourceUrl(ConstantService.vocp_api_vlink+row.recordFile+"?appId="+this.params.appId+"&timestamp="+this.callRecordsCdrObTotal.timestamp+"&sign="+this.callRecordsCdrObTotal.sign)
    ...
    downfile(evt,row,mark) {
        var strfile = row.recordFile+"?appId="+this.params.appId+"&timestamp="+this.callRecordsCdrObTotal.timestamp+"&sign="+this.callRecordsCdrObTotal.sign
